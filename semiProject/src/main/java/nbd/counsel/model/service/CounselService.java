...
public class CounselService {
	...
	public CounselPageData selectSubjectList(String counselSubject, int reqPage, int categoryIndex) {
		...
		for (int i = 0; i < pageNaviSize; i++) {
			if (pageNo == reqPage) {
				pageNavi += "<li>";
				pageNavi += "<a class='page-item active-page' href='/counselSubjectList.do?clickIndex=1"
						+ "&categoryIndex=" + categoryIndex 
						+ "&counselSubject=" + counselSubject
						+ "&reqPage=" + pageNo + "'>";
				pageNavi += pageNo;
		...
		CounselPageData cpd
		= new CounselPageData(list, pageNavi);
	...
	public CounselViewData selectOneCounsel(int counselNo) {
		...
		if(result > 0) {
			...
			= dao.selectCounselComment(conn, counselNo);
			
			CounselViewData cvd
			= new CounselViewData(c, comment);
	...
	// SELECT ROWNUM, MEMBER_ID, COUNSEL_TITLE, COUNSEL_CONTENT, COUNSEL_ANSWER FROM (SELECT * FROM HEALTH_COUNSEL_TBL WHERE MEMBER_ID='USER117' ORDER BY COUNSEL_NO DESC);
		Connection conn = JDBCTemplate.getConnection();
	...
	public ArrayList<Counsel> getMyAnswer(int memberNo, int startNum, int endNum) {
		// SELECT ROWNUM, R.* FROM (SELECT * FROM HEALTH_COUNSEL_TBL WHERE COUNSEL_NO IN (SELECT COUNSEL_NO FROM HEALTH_COUNSEL_COM_TBL WHERE MEMBER_ID='DOC1') ORDER BY COUNSEL_NO DESC)R WHERE ROWNUM BETWEEN 1 AND 3;
		Connection conn = JDBCTemplate.getConnection();
